...
def main():
    ...
    try:
        ...
        df_humano_final, df_robo_mestre = processar_dados(all_dataframes, config)
        logging.info("--- ESTÁGIO 2 CONCLUÍDO ---")
        
        output_dir = Path(config.get('PATHS', 'output_dir', fallback='./data_output'))
        pasta_do_dia = output_dir / datetime.now().strftime('%d_%m_%Y')
        pasta_do_dia.mkdir(exist_ok=True)
        
        if df_humano_final.empty and df_robo_mestre.empty:
            ...
            if not df_humano_final.empty:
                exportar_dados_humanos(df_humano_final, config, pasta_do_dia)
            
            if not df_robo_mestre.empty:
                gerar_arquivo_robo_mestre(df_robo_mestre, config, pasta_do_dia)
            logging.info("--- ESTÁGIO 3 CONCLUÍDO ---")
        
        logging.info("--- ESTÁGIO 4: Padronizando dados para o Excel e Power BI ---")
        ...
        logging.info("--- ESTÁGIO 6 CONCLUÍDO ---")
